...
contract Storage is IStorage {
    ...
    event AssetCreated(uint256 _assetId);
    event AssetAttributeSet(uint256 _assetId, string _attrName);

    address private _owner;
    mapping(uint256 => Asset) private _assetList;
    ...
    function getAssetContentForId(uint256 _assetId) public view override returns (bytes memory) {
    ...
    function getAssetKeysForId(uint256 _assetId) public view override returns (bytes32[] memory) {
    ...
    function getAssetSize(uint256 _assetId) public view override returns (uint64) {
        return _assetList[_assetId]._byteSize;
    }

    function getAssetAttribute(uint256 _assetId, string calldata _attrName) public view override returns (Attr memory _attr) {
