...
describe('AuctionMachine Workflow Tests', () => {
  ...
  before(async () => {
    ...
    auctionMachine = await auctionMachineFactory
      .connect(deployer)
      .deploy(3, auctionDuration, jbxProjectId, mockDirectory.address, token.address);

    token.connect(deployer).addMinter(auctionMachine.address);
  });

  it('Bootstrap the first auction with a bid', async () => {
    await expect(auctionMachine.connect(accounts[0]).bid({ value: lowPrice })).to.emit(
      auctionMachine,
      'AuctionStarted',
    );
  ...
  it('Place bids on auction', async () => {
    await expect(
      auctionMachine.connect(accounts[0]).bid({ value: lowPrice.mul(2) }),
  ...
  it('Settle auction with bids', async () => {
    await expect(
      auctionMachine.connect(accounts[1]).bid({ value: tokenUnitPrice }),
    ).to.be.revertedWith('INVALID_BID()');
    ...
    await expect(
      auctionMachine.connect(accounts[1]).bid({ value: tokenUnitPrice.mul(3) }),
    ...
    await expect(auctionMachine.connect(accounts[0]).settle()).to.be.revertedWith(
      'AUCTION_ACTIVE()',
    );
  });
  ...
  it('Exhaust auction availability', async () => {
    ...
    await expect(
      auctionMachine.connect(accounts[0]).bid({ value: tokenUnitPrice }),
    ).to.be.revertedWith('SUPPLY_EXHAUSTED()');
  });

  it('Transfer owned token', async () => {
    await expect(
      auctionMachine.connect(accounts[0]).recoverToken(token.address, 2),
    ).to.be.revertedWith('Ownable: caller is not the owner');

    await expect(auctionMachine.connect(deployer).recoverToken(accounts[3].address, 2))
      .to.emit(token, 'Transfer')
      .withArgs(auctionMachine.address, accounts[3].address, 2);

    await expect(
      auctionMachine.connect(deployer).recoverToken(accounts[3].address, 2),
