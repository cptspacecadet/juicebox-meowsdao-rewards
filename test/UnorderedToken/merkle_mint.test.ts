...
describe('MEOWs DAO Token Mint Tests: Merkle Tree', () => {
  ...
  it('User mints', async () => {
    ...
    await expect(
      token.connect(accounts[1]).merkleMint(merkleItem.index, merkleItem.data, merkleItem.proof),
  ...
  it('User mints: failure, invalid proof', async () => {
    const merkleItem = merkleData.claims[accounts[1].address];
    await expect(
      token.connect(accounts[0]).merkleMint(merkleItem.index, merkleItem.data, merkleItem.proof),
  ...
  it('User mints first: failure, claims exhausted', async () => {
    ...
    for (let i = 0; i < Number(merkleItem.data); i++) {
      await token
        .connect(accounts[2])
        .merkleMint(merkleItem.index, merkleItem.data, merkleItem.proof);
    }

    await expect(
      token.connect(accounts[2]).merkleMint(merkleItem.index, merkleItem.data, merkleItem.proof),
  ...
  it('User mints: payment failure', async () => {
    ...
    await expect(
      token.connect(accounts[3])['mint()']({ value: tokenUnitPrice }),
