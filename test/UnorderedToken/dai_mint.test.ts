...
describe('MEOWs DAO Token Mint Tests: DAI', () => {
  ...
  before(async () => {
    ...
    mockDai = await deployMockContractFromAddress(
    ...
    mockWeth = await deployMockContractFromAddress(
    ...
    await mockDirectory.isTerminalOf
      .whenCalledWith(jbxProjectId, daiTerminal.address)
      .returns(true);
    await mockDirectory.primaryTerminalOf
      .whenCalledWith(jbxProjectId, mockDai.address)
      .returns(daiTerminal.address);
  ...
  it('User mints second: fail due to unapproved token', async () => {
    ...
    await expect(
      token.connect(accounts[0])['mint(address)'](mockDai.address, { value: tokenUnitPrice }),
