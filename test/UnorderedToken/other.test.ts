...
describe('UnorderedToken "Other" Tests', () => {
    ...
    it('updatePaymentTokenList', async () => {
        ...
        await expect(token.connect(accounts[0]).updatePaymentTokenList(daiAddress, true))
            .to.be.reverted;

        await expect(token.connect(deployer).updatePaymentTokenList(daiAddress, false))
            .to.be.not.reverted;
    });

    it('updatePaymentTokenParams', async () => {
        await expect(token.connect(accounts[0]).updatePaymentTokenParams(true, 1000))
            .to.be.reverted;

        await expect(token.connect(deployer).updatePaymentTokenParams(true, 11000))
            .to.be.revertedWith('INVALID_MARGIN()');

        await expect(token.connect(deployer).updatePaymentTokenParams(true, 1000))
            .to.be.not.reverted;
    });
});
