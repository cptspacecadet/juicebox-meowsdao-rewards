...
describe('Simple Token "Other" Tests', () => {
  ...
  it('setProvenanceHash', async () => {
    ...
    await expect(token.connect(deployer).setProvenanceHash('0xdeadbeef')).to.be.revertedWith(
      'PROVENANCE_REASSIGNMENT()',
    );
  });
  ...
  it('setBaseURI', async () => {
    ...
    await expect(token.connect(deployer).setBaseURI(tokenBaseUri, false)).to.be.revertedWith(
      'ALREADY_REVEALED()',
    );
  });
});
