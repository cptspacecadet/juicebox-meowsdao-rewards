...
describe('TraitsChainToken Mint Tests', () => {
  ...
  it('Admin mints 7: supply failure', async () => {
    await expect(token.connect(deployer).mintFor(accounts[2].address)).to.be.revertedWith(
      'SUPPLY_EXHAUSTED()',
    );
  });

  it('Admin updates storage address', async () => {
    await expect(token.connect(deployer).setAssets(ethers.constants.AddressZero)).to.not.be
      .reverted;
  });
});
