...
describe('TraitsGatewayToken Mint Tests', () => {
  ...
  it('User fails to mint a second token with 0 eth', async () => {
    await expect(token.connect(accounts[0])['mint()']({ value: 0 })).to.be.revertedWith(
      `INCORRECT_PAYMENT(${tokenUnitPrice.toString()})`,
    );
  });
  ...
  it('Pre/post reveal tokenURI', async () => {
    ...
    await expect(token.connect(deployer).setBaseURI('', true)).to.not.be.reverted;
  ...
  it('Admin updates ipfs gateway', async () => {
    ...
    await expect(token.connect(deployer).setIPFSGatewayURI(gateway)).to.not.be.reverted;
  });

  it('Admin updates ipfs cid', async () => {
    const cid = 'c0ffee/';
    await expect(token.connect(deployer).setIPFSRoot(cid)).to.not.be.reverted;
  });
});
